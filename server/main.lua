...
AddEventHandler('apartments:server:CreateApartment', function(type, label)
    ...
    TriggerClientEvent('QBCore:Notify', src, "You got an apartment ("..label..")")
...
AddEventHandler('apartments:server:AddObject', function(apartmentId, apartment, zoffset, xoffset)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if ApartmentObjects[apartment] ~= nil and ApartmentObjects[apartment].apartments ~= nil and ApartmentObjects[apartment].apartments[apartmentId] ~= nil then
        ...
        if ApartmentObjects[apartment] ~= nil and ApartmentObjects[apartment].apartments ~= nil then
            ...
            ApartmentObjects[apartment].apartments[apartmentId].zoffset = zoffset
            ApartmentObjects[apartment].apartments[apartmentId].xoffset = xoffset
...
QBCore.Functions.CreateCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    ...
    if ApartmentObjects ~= nil then
        for k, v in pairs(ApartmentObjects) do
            if (ApartmentObjects[k].apartments[apartmentId] ~= nil and tonumber(ApartmentObjects[k].apartments[apartmentId].zoffset) ~= 0) then
                zretval = tonumber(ApartmentObjects[k].apartments[apartmentId].zoffset)
                xretval = tonumber(ApartmentObjects[k].apartments[apartmentId].xoffset)
            end
        end
    end
    cb(zretval, xretval)
...
QBCore.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
    local zretval = Apartments.SpawnZOffset
    local xretval = 0

    while Apartments.Locations[apartment].doorclosed do
		Citizen.Wait(50)
	end
    
    Apartments.Locations[apartment].doorclosed = true
    ...
    if ApartmentObjects ~= nil and ApartmentObjects[apartment] ~= nil and ApartmentObjects[apartment].apartments ~= nil then
        for k, v in pairs(ApartmentObjects[apartment].apartments) do
            ...
            --[[ if (ApartmentObjects[apartment].apartments[k] ~= nil) then
                zretval = ApartmentObjects[apartment].apartments[k].zoffset + Apartments.SpawnZOffset
                xretval = ApartmentObjects[apartment].apartments[k].xoffset + Apartments.SpawnXOffset
            end ]]
            if (ApartmentObjects[apartment].apartments[k].zoffset >= 150) then
                zretval = ApartmentObjects[apartment].apartments[k].zoffset + 30
                xretval = ApartmentObjects[apartment].apartments[k].xoffset + 0
            else
                zretval = ApartmentObjects[apartment].apartments[k].zoffset - ApartmentObjects[apartment].apartments[k].zoffset + 30
                xretval = ApartmentObjects[apartment].apartments[k].xoffset + 30
            end
        end
    end
    --print("Position: " .. xretval .. ", " .. zretval)
    Apartments.Locations[apartment].doorclosed = false
    cb(zretval, xretval)
end)
